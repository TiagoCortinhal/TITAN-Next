...
class LaserScanVis:
    ...
    def reset(self):
        ...
        self.scan_view = vispy.scene.widgets.ViewBox(
           border_color='white', parent=self.canvas.scene,bgcolor='white')
        ...
        if self.semantics:
            ...
            self.sem_view = vispy.scene.widgets.ViewBox(
                border_color='white', parent=self.canvas.scene,bgcolor='white')
            self.grid.add_widget(self.sem_view)
        ...
        if self.semantics:
            ...
            self.sem_img_vis = visuals.Image()
    ...
    def update_scan(self):
        ...
        #io.write_png('/Users/tiacor/Downloads/data/sequences/13/projected_images/pc2/pc_{0:05d}.png'.format(self.offset),
        #             img)

        if self.semantics:
            self.sem_img_vis.set_data(self.scan.proj_sem_color[..., ::-1])
            self.sem_img_vis.update()
            #plt.imsave('/Users/tiacor/Downloads/data/sequences/13/projected_images/proj2/proj_{0:05d}.png'.format(self.offset),self.scan.proj_sem_color[..., ::-1])
