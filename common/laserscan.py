...
class LaserScan:
    ...
    def open_scan(self, filename):
        ...
        # cam2_points = self.calib.T_cam3_velo.dot(scan.T).T
        # self.idx = cam2_points[:, 2] > 0
        #cam2_points = cam2_points[idx]
        #scan = scan[idx]

        #cam2_points = cam2_points[:, :3] / cam2_points[:, 2:3]
        #intrinsic_cam2 = self.calib.K_cam2
        #cam2_points = intrinsic_cam2.dot(cam2_points.T).T[:, [1, 0]]
        #cam2_points = cam2_points.astype(int)
        #img_width = 1241
        #img_height = 376
        #self.idx = np.where((cam2_points[:,0] < img_height) & (cam2_points[:,1] < img_width)
        #                    & (cam2_points[:,0]>0) & (cam2_points[:,1]>0))[0]
        #cam2_points = cam2_points[:, :3] / cam2_points[:, 2:3]
        #intrinsic_cam2 = self.calib.K_cam3
        #cam2_points = intrinsic_cam2.dot(cam2_points.T).T[:, [1, 0]]
        #self.idx2 = cam2_points[:]
    ...
    def do_range_projection(self):
        ...
        proj_x = 0.5 * (yaw / (np.pi/4) + 1.0) # in [0.0, 1.0]
